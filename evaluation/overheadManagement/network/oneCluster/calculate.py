...
# Filtere die Pods, die "overhead" im Namen haben, heraus und speichere ihre Namen
overhead_pods = all_data[all_data['pod'].str.contains('overhead')]['pod'].unique()
filtered_data = all_data[~all_data['pod'].str.contains('overhead')]

# Filtere die Pods, die mit "kube-multus" beginnen, bei den spezifischen Hauptverzeichnissen
additional_filtered_pods = []
for framework in ['kubernetes', 'karmada', 'ocm']:
    pods_to_filter = filtered_data[(filtered_data['framework'] == framework) & (filtered_data['pod'].str.startswith('kube-multus'))]['pod'].unique()
    additional_filtered_pods.extend(pods_to_filter)
    filtered_data = filtered_data[~((filtered_data['framework'] == framework) & (filtered_data['pod'].str.startswith('kube-multus')))]

# Berechne den Durchschnittswert für jeden Pod über alle Unterverzeichnisse hinweg
final_average_df = filtered_data.groupby(['framework', 'namespace', 'pod', 'container', 'type'])['value'].mean().reset_index()
...
print(f'Pods, die nicht mitgespeichert wurden (overhead): {overhead_pods}')
print(f'Pods, die nicht mitgespeichert wurden (kube-multus): {additional_filtered_pods}')
