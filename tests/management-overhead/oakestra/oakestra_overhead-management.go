...
func generateJSON(config Config, templatePath string) (string, error) {
...
func main() {
	fmt.Println("Start Test Overhead Management")
	template := "./overhead-management.template.json"
	SLA_FILE := "./overhead-management.json"

	config = Config{
		Duration: 60,
		VU:       1,
		ServerIP: "10.30.55.54", //,
	}

	content, _ := generateJSON(config, template)
	err := os.WriteFile(SLA_FILE, []byte(content), 0644)
	if err != nil {
...
func sendDeployRequest(url string) {
	token, _ := getLogin()
...
func deployOakestra() {
	url_client_request := fmt.Sprintf("http://%s/api/service/%s/instance", SYSTEM_MANAGER_URL, CLIENT_SERVICE_ID)
	fmt.Println(url_client_request)
	url_server_request := fmt.Sprintf("http://%s/api/service/%s/instance", SYSTEM_MANAGER_URL, SERVER_SERVICE_ID)
	fmt.Println(url_server_request)

	// Client Deploy
	sendDeployRequest(url_client_request)

	// Server Deploy
	sendDeployRequest(url_server_request)
}

func undeployOakestra() {

	// Only one instance with 10 replicas gets deployed
	instance_number := 0

	url_client_request := fmt.Sprintf("http://%s/api/service/%s/instance/%d", SYSTEM_MANAGER_URL, CLIENT_SERVICE_ID, instance_number)
	fmt.Println(url_client_request)
	url_server_request := fmt.Sprintf("http://%s/api/service/%s/instance/%d", SYSTEM_MANAGER_URL, SERVER_SERVICE_ID, instance_number)
	fmt.Println(url_server_request)

	// Client Delete
	deleteRequest(url_client_request)

	// Server Delete
	deleteRequest(url_server_request)
}
