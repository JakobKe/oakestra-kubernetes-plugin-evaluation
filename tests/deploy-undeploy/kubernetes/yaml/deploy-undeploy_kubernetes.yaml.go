...
func main() {
	...
	deploymentInfos := deploy(numDeployments)
	...
	go func() {
		...
		deletedDeployments, err := undeploy()
		if err != nil {
			errChan <- err
			return
		}
	...
	writeInfoToCSV("kubernetes.yaml.deploy.undeploy.csv", deploymentInfos, deletedDeployments, cleanedPods)
}
...
func createKubernetesYaml(name string) (string, error) {
...
func execKubernetesFile(filePath string, command string) {
...
func applyYAMLFilesInFolder(folder string) map[string]DeploymentInfo {
	...
	for _, file := range files {
		...
		execKubernetesFile(filePath, "apply")
...
func deploy(numDeployments int) map[string]DeploymentInfo {
	dir := "./deploymentFiles"
	...
	for i := 1; i <= numDeployments; i++ {
		...
		yamlContent, err := createKubernetesYaml(deploymentName)
		if err != nil {
...
func undeploy() (map[string]time.Time, error) {
	dir := "./deploymentFiles"
...
func deleteYAMLFilesInFolder(folder string) map[string]time.Time {
	...
	for _, file := range files {
		...
		filePath := folder + "/" + file.Name()
		execKubernetesFile(filePath, "delete")
