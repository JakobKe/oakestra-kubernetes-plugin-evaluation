...
def main():
    ...
    query = 'avg(rate(node_cpu_seconds_total[5m]))[5m:1s]'
    filename = 'node_cpu_seconds_total.csv'  
    data = query_prometheus(query, prometheus_url)
    save_to_csv(data, directory, filename)
    
    query = ' avg by (instance) (rate(node_cpu_seconds_total{mode="idle"}[5m]))[5m:1s]'
    filename = 'node_cpu_seconds_total_by_instance.csv'  
    data = query_prometheus(query, prometheus_url)
    save_to_csv(data, directory, filename)
    ...
    query = 'avg by (instance) (node_memory_MemTotal_bytes)'
    filename = 'node_memory_MemTotal_bytes_by_instance.csv'  
    data = query_prometheus_interval(query)
    save_to_csv(data, directory, filename)
    ...
    query = 'sum(rate(container_cpu_usage_seconds_total[5m]))[5m:1s]'
